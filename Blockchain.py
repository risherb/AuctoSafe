...
class Blockchain:
    ...
    #generate deniable encryption keys
    def generateDMEKeys(self):
        if os.path.exists("keys/keys.txt"):
            f = open('keys/keys.txt', 'rb')
            keys = pickle.load(f)
            f.close()
            private_key = keys[0]
            public_key = keys[1]
            ...
            f = open('keys/keys.txt', 'wb')
            pickle.dump(keys, f)
            f.close()  
        return private_key, public_key

    #Deniable encryption will encrypt data using plain text adn public key
    def deniableEncrypt(self, plainText, public_key):
        dme_encrypt = encrypt(public_key, plainText.encode())
        dme_encrypt = str(base64.b64encode(dme_encrypt),'utf-8')
        return dme_encrypt

    #denyable encryption will decrypt data using private key and encrypted text
    def deniableDecrypt(self, encrypt, private_key):
        encrypt = base64.b64decode(encrypt)
        dme_decrypt = decrypt(private_key, encrypt)
        return dme_decrypt
